// OnboardingView.swift — SmartCart/Views/OnboardingView.swift
// First-launch flow: Step 1 = Store picker, Step 2 = Postal code, Step 3 = Notifications.
// onComplete() sets onboarding_complete flag and switches to HomeView.
...
struct OnboardingView: View {
    ...
    private var storePickerStep: some View {
        VStack(alignment: .leading, spacing: 24) {
            OnboardingHeader(title: "Which stores do you shop at?",
                             subtitle: "SmartCart tracks prices and flyers for your chosen stores.")
            FlowLayout(spacing: 10) {
                ForEach(availableStores, id: \.self) { store in
                    StoreChip(name: store, isSelected: selectedStores.contains(store)) {
    ...
    private var postalCodeStep: some View {
        VStack(alignment: .leading, spacing: 24) {
            OnboardingHeader(title: "What\u2019s your postal code?",
            ...
            Text("You can skip this and add it later in Settings.")
                .font(.system(size: 13)).foregroundStyle(.secondary).padding(.horizontal, 20)
            Spacer()
            HStack(spacing: 12) {
    ...
    private var notificationStep: some View {
        VStack(alignment: .leading, spacing: 24) {
            OnboardingHeader(title: "Get price alerts",
                             subtitle: "Notified when items hit a new low or go on sale. Max 3 alerts/day.")
